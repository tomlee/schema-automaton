...
| No `NaN`/`Infinity` | JSON | `write_json` does not raise in lenient (default) mode — it substitutes `null` at the leaf so the output is always valid JSON (rather than the old behavior of emitting the literal tokens `NaN`/`Infinity`, which is not valid per the JSON spec). `check_json` reports this as `float.special`, an error-severity adjustment whose message notes the substitution, so a caller checking the report learns about it without `write_json` itself failing. `strict=True` raises `WriteError` instead of substituting. |
